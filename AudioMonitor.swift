...
class AudioMonitor: AURenderCallbackDelegate {
                ...
                nc.addObserver(forName: .NSExtensionHostDidEnterBackground, object: nil, queue: .main) { _ in
                    ...
                    self.backgroundDisableTimer = Timer.scheduledTimer(withTimeInterval: Double(self.backgroundDisableTimerDelay), repeats: true, block: { _ in
                        if self.isRunning && UIDevice.current.batteryState == .unplugged {
                            ...
                            print("ToneSensor was disabled while unplugged after a \(self.backgroundDisableTimerDelay) second timeout. Set ToneSensor.backgroundDisableTimerDelay to adjust this delay.")
    ...
    ///
    /// - localizationKey: ToneSensor.currentSample
    ...
    /// Tells the ToneSensor to start collecting data.
    ///
    /// - localizationKey: ToneSensor.start()
    public func start() {
    ...
    /// Tells the ToneSensor to stop collecting data.
    ///
    /// - localizationKey: ToneSensor.stop()
    public func stop() {
        let component = audioComponent!,
    ...
    ///
    /// - localizationKey: ToneSensor.setOnUpdateHandler(_:)
    ...
    var inputSimTone = Tone(pitch: 0.0, volume: 0.0) {
        didSet {
            ...
            //axToneOutput.play(tone: inputSimTone)
